...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--exp-name",  "-e", type=str, required=True,
        help="The job name and path to logging file (exp_name.log)."
    )

    parser.add_argument(
        "--max-time",  "-t", type=str, default="03:00:00",
        help="max time for running job. (Default: 3h)"
    )
    parser.add_argument(
    ...
    parser.add_argument("--num-cpus", type=int, default=0, 
        help="Number of CPUs to use in each node. (Default: 0)")
    parser.add_argument(
        "--num-gpus", type=int, default=0,
        help="Number of GPUs to use in each node. (Default: 0)"
    )
    parser.add_argument(
        "--partition", "-p", type=str, default="batch",
    )
    parser.add_argument(
    ...
    text = text.replace(MAX_TIME,  str(args.max_time))
